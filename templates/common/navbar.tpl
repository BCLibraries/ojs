	...
	<ul class="menu">
		...
		{foreach from=$navMenuItems item=navItem key=navItemKey}
			{if $navItem.url != '' && $navItem.name != ''}
				<li class="navItem" id="navItem-{$navItemKey|escape}"><a href="{if $navItem.isAbsolute}{$navItem.url|escape}{else}{$baseUrl}{$navItem.url|escape}{/if}">{if $navItem.isLiteral}{$navItem.name|escape|strip:'&nbsp;'}{else}{translate key=$navItem.name|strip:'&nbsp;'}{/if}</a></li>
			{/if}
		{/foreach}
	</ul>
</div>
